...
def svm_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  scores = X.dot(W)
  margins = np.maximum(0,scores.T-scores[np.arange(num_train),y]+1) # delta =1
  margins[y,np.arange(num_train)] = 0
  loss = np.sum(margins) + 0.5 * reg * np.sum(W * W)
  loss /= num_train
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  mask[margins>0] = 1
  toCorrect = np.sum(mask,axis=0)
  mask[y,range(num_train)] = -toCorrect[range(num_train)]
  dW=mask.dot(X).T
